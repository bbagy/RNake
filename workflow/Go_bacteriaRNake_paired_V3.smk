...
# Change this part to accommodate R2 reads
READS_R1 = glob.glob(READ_DIR + '/*R1*.fastq.gz')
READS_R2 = [read.replace('R1', 'R2') for read in READS_R1]
SAMPLES = [read.split('/')[-1].split('_R1_001')[0] for read in READS_R1]
...
rule trim_reads:
    input:
        r1 = f"{READ_DIR}/{{sample}}_R1_001.fastq.gz",
        r2 = f"{READ_DIR}/{{sample}}_R2_001.fastq.gz"
    output:
    ...
    shell:
        ...
        trimmomatic PE -threads 10 -summary {master_dir}/1_trim/{wildcards.sample}.statssummary -phred33 {input.r1} {input.r2} {output.trimmed_r1_paired} {output.trimmed_r1_unpaired} {output.trimmed_r2_paired} {output.trimmed_r2_unpaired} LEADING:3 TRAILING:3 SLIDINGWINDOW:4:20 MINLEN:36
        """
